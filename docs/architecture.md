...
`andx-project inspect PATH` is the read-only interface for the generated
project contract. Its implementation keeps envelope classification, adapter
pointer validation, starter-marker parsing, and the existing continuity audit
behind that one command. Callers do not need to reproduce those checks.

Inspection never creates a snapshot, repairs a path, or rewrites project
content. It exits `0` when the current contract is healthy and `3` when it
finds an actionable contract or continuity issue. Usage and operational errors
remain distinct. Version differences are reported as facts; only a missing or
malformed marker is a finding. Migration availability is reported separately,
and no migration is implied merely because an installed command has a newer
version.

The continuity check reuses the bundled audit through its command interface.
It runs only when the inspected path is itself a Git repository root, so Git
cannot silently resolve a parent checkout and report health for the wrong
project.
