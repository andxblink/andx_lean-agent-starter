...
- GitHub Actions now uses `actions/checkout@v7`, whose Node 24 runtime replaces
...
- Read-only `andx-project inspect` reporting the starter marker, missing or
  wrongly typed envelope paths, adapter pointer drift, continuity audit
  health, and available migrations. Actionable findings return exit status
  `3`.

### Fixed

- `andx-project inspect` no longer expands empty arrays while nounset mode is
  active, preserving compatibility with the Bash 3.2 variant on GitHub's macOS
  runner.
